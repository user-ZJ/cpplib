...
int ONNXEncoderEngine::loadModel(const std::string &modelPath, bool is_crypto) {
...
int ONNXEncoderEngine::infer(const CTensorfl &input, CTensorfl *out) {
  LOG(INFO) << "ONNXEncoderEngine::infer";
  try {
    if (!is_init_) {
    ...
    LOG(INFO) << "ONNXEncoderEngine::infer end";
    return 0;
