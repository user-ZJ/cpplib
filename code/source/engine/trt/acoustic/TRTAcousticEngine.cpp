...
int TRTAcousticEngine::loadModel(const std::string &encoderPath,
...
int TRTAcousticEngine::infer(const CTensorfl &input,int bss_len,CTensorfl *out) {
  LOG(INFO) << "TRTAcousticEngine::infer";
  try {
    if (!is_init_) {
    ...
    decoder_iter.infer(memories,out);
    ...
    // }
    LOG(INFO) << "TRTAcousticEngine::infer end";
    return 0;
