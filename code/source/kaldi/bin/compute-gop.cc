...
/**
   ...
   "Improved mispronunciation detection with deep neural network trained
   ...
   classifier-based approach archives better performance than the GOP-based
   approach.
   ...
   where the Log Posterior Ratio (LPR) between phone p_j and p_i is defined as:
     LPR(p_j|p_i) = \log p(p_j|\mathbf o; t_s, t_e) - \log p(p_i|\mathbf o; t_s,
   t_e)
 */
...
/** ComputeLpps compute log posteriors for pure-phones by sum the posterior
    of the states belonging to those triphones whose current phone is the
   canonical phone:
  ...
  try {
    ...
    int32 num_done = 0;
    {
      ...
      if (alignment.size() != probs.NumRows()) {
        ...
        if (frame_num > probs.NumRows())
          frame_num = probs.NumRows();
      }
      ...
      int32 duration = 0;
      Vector<BaseFloat> phone_level_feat(1 +
                                         phone_num * 2); // [phone LPPs LPRs]
      ...
      for (int32 i = 0; i < frame_num; i++) {
        ...
        if (next_phone_id != cur_phone_id) {
          int32 phone_id =
              phone_map.empty() ? cur_phone_id : phone_map[cur_phone_id];

          // The current phone's feature have been ready
          lpp_part.Scale(1.0 / duration);

          // LPR(p_j|p_i)=\log p(p_j|\mathbf o; t_s, t_e)-\log p(p_i|\mathbf o;
          // t_s, t_e)
          for (int k = 0; k < phone_num; k++)
            phone_level_feat(1 + phone_num + k) =
                lpp_part(phone_id) - lpp_part(k);
