...
void WordBoundaryInfo::Init(const std::vector<std::pair<int, std::string>> &word_boundary_int) {
  int32 p = 0;
  std::string t;
  for(auto &w:word_boundary_int) {
    std::tie(p,t) = w;
    KALDI_ASSERT(p > 0);
    if (phone_to_type.size() <= static_cast<size_t>(p))
      phone_to_type.resize(p+1, kNoPhone);
    if (t == "nonword") phone_to_type[p] = kNonWordPhone;
      ...
      KALDI_ERR << "Invalid line in word-boundary file: " << p<<" "<<t;
  }
  if (phone_to_type.empty())
    KALDI_ERR << "Empty word-boundary file";
}
