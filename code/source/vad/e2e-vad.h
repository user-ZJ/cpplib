...
enum class FrameState { kFrameStateInvalid = -1, kFrameStateSpeech = 1, kFrameStateSil = 0 };
...
enum class VadDetectMode { kVadSingleUtteranceDetectMode = 0, kVadMutipleUtteranceDetectMode = 1 };
...
class VADXOptions {
 public:
  ...
  explicit VADXOptions(int sr = 16000, int dm = static_cast<int>(VadDetectMode::kVadMutipleUtteranceDetectMode),
                       int sm = 0, int mset = 800, int msst = 3000, bool dspd = true, bool depd = true, int wsm = 200,
                       int ststh = 150, int sttsh = 150, float s2nr = 1.0, int de = 1, int lbtps = 200, int latsp = 100,
                       int mss = 15000, int nebs = 8, int dbs = 4, float st = -100.0, int nfnus = 100,
                       float dt = -100.0, float snt = 0.9, float fept = 1e-4, int spn = 1, std::vector<int> spids = {0},
                       float sntl = -0.1, float snth = 0.3, bool ofp = false, int fim = 10, int flm = 25) :
    sample_rate(sr),
    detect_mode(dm), snr_mode(sm), max_end_silence_time(mset), max_start_silence_time(msst),
    do_start_point_detection(dspd), do_end_point_detection(depd), window_size_ms(wsm), sil_to_speech_time_thres(ststh),
    speech_to_sil_time_thres(sttsh), speech_2_noise_ratio(s2nr), do_extend(de), lookback_time_start_point(lbtps),
    lookahead_time_end_point(latsp), max_single_segment_time(mss), nn_eval_block_size(nebs), dcd_block_size(dbs),
    snr_thres(st), noise_frame_num_used_for_snr(nfnus), decibel_thres(dt), speech_noise_thres(snt),
    fe_prior_thres(fept), silence_pdf_num(spn), sil_pdf_ids(std::move(spids)), speech_noise_thresh_low(sntl),
    speech_noise_thresh_high(snth), output_frame_probs(ofp), frame_in_ms(fim), frame_length_ms(flm) {}
};
...
class E2EVadSpeechBufWithDoa {
 public:
  ...
  E2EVadSpeechBufWithDoa() :
    start_ms(0), end_ms(0), buffer(), contain_seg_start_point(false), contain_seg_end_point(false), doa(0) {}
...
class E2EVadFrameProb {
 public:
  ...
  E2EVadFrameProb() : noise_prob(0.0), speech_prob(0.0), score(0.0), frame_id(0), frm_state(0) {}
};
...
class WindowDetector {
 public:
  ...
  WindowDetector(int window_size_ms, int sil_to_speech_time, int speech_to_sil_time, int frame_size_ms) :
    window_size_ms(window_size_ms), sil_to_speech_time(sil_to_speech_time), speech_to_sil_time(speech_to_sil_time),
    frame_size_ms(frame_size_ms), win_size_frame(window_size_ms / frame_size_ms), win_sum(0),
    win_state(std::vector<int>(win_size_frame, 0)), cur_win_pos(0), pre_frame_state(FrameState::kFrameStateSil),
    cur_frame_state(FrameState::kFrameStateSil), sil_to_speech_frmcnt_thres(sil_to_speech_time / frame_size_ms),
    speech_to_sil_frmcnt_thres(speech_to_sil_time / frame_size_ms), voice_last_frame_count(0),
    noise_last_frame_count(0), hydre_frame_count(0) {}
  ...
  AudioChangeState DetectOneFrame(FrameState frameState, int frame_count) {
    ...
    if (pre_frame_state == FrameState::kFrameStateSil) { return AudioChangeState::kChangeStateSil2Sil; }
...
class E2EVadModel {
 public:
  E2EVadModel() {
    ...
    //    this->frame_probs = [];
    this->max_end_sil_frame_cnt_thresh = this->vad_opts.max_end_silence_time - this->vad_opts.speech_to_sil_time_thres;
  ...
  std::vector<std::vector<int>> operator()(const std::vector<std::vector<float>> &score,
    ...
    if (output_data_buf.size() > 0) {
      for (size_t i = output_data_buf_offset; i < output_data_buf.size(); i++) {
        ...
        int end_ms;
        if (online) {
          if (!output_data_buf[i].contain_seg_start_point) { continue; }
          if (!next_seg && !output_data_buf[i].contain_seg_end_point) { continue; }
          ...
          if (!is_final && (!output_data_buf[i].contain_seg_start_point || !output_data_buf[i].contain_seg_end_point)) {
            continue;
          }
 ...
 private:
  ...
  void PopDataToOutputBuf(int start_frm, int frm_cnt, bool first_frm_is_start_point, bool last_frm_is_end_point,
    ...
    if (end_point_is_sent_end) { expected_sample_number = std::max(expected_sample_number, data_buf_size); }
    if (data_buf_size < expected_sample_number) { std::cout << "error in calling pop data_buf\n"; }
    ...
    if (cur_seg.end_ms != start_frm * vad_opts.frame_in_ms) { std::cout << "warning\n"; }
    ...
    if (cur_seg.end_ms != start_frm * vad_opts.frame_in_ms) { std::cout << "Something wrong with the VAD algorithm\n"; }
  ...
  void OnSilenceDetected(int valid_frame) {
    ...
    if (vad_state_machine == VadStateMachine::kVadInStateStartPointNotDetected) { PopDataBufTillFrame(valid_frame); }
  ...
  int LatencyFrmNumAtStartPoint() {
    ...
    if (vad_opts.do_extend) { vad_latency += int(vad_opts.lookback_time_start_point / vad_opts.frame_in_ms); }
    return vad_latency;
  }
  ...
  FrameState GetFrameState(int t) {
    ...
    if (exp(speech_prob) >= exp(noise_prob) + speech_noise_thres) {
      ...
      if (noise_average_decibel < -99.9) {
        ...
        noise_average_decibel = (cur_decibel + noise_average_decibel * (vad_opts.noise_frame_num_used_for_snr - 1))
  ...
  int DetectCommonFrames() {
    if (vad_state_machine == VadStateMachine::kVadInStateEndPointDetected) { return 0; }
    ...
    if (AudioChangeState::kChangeStateSil2Speech == state_change) {
      ...
      if (vad_state_machine == VadStateMachine::kVadInStateStartPointNotDetected) {
        ...
        if (continous_silence_frame_count * frm_shift_in_ms >= max_end_sil_frame_cnt_thresh) {
          ...
          vad_state_machine = VadStateMachine::kVadInStateEndPointDetected;
        } else if (cur_frm_idx - confirmed_start_frame + 1 > vad_opts.max_single_segment_time / frm_shift_in_ms) {
          OnVoiceEnd(cur_frm_idx, false, false);
          vad_state_machine = VadStateMachine::kVadInStateEndPointDetected;
