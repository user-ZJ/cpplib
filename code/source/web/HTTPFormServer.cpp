...
public:
  ...
  void handlePart(const MessageHeader &header, std::istream &stream) {
    ...
    if (header.has("Content-Disposition")) {
	  std::cout<<"Content-Disposition:"<<header.get("Content-Disposition", "(unspecified)")<<std::endl;
      std::string disp;
	  ...
	  std::cout<<"name:"<<_name<<" filename:"<<_fileName<<std::endl;
    }
...
protected:
  ...
  int main(const std::vector<std::string> &args) {
    if (_helpRequested) {
      ...
      HTTPServer srv(new FormRequestHandlerFactory, svs, pParams);
