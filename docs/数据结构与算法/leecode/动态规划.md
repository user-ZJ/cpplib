...
```text
给定一个数字，我们按照如下规则把它翻译为字符串：0翻译成a，1翻译成b，……，11翻译成l，……，25翻译成z，一个数字可能有多个翻译。例如12258有5种不同的翻译。请实现一个函数，计算一个数字有多少种不同的翻译方法
```
...
```text
在一个mxn的棋盘的每一格都放有一个礼物，每个礼物都有一定的价值（大于0），从期盼的左上角开始拿格子里的礼物，并每次向左或者向下移动一格，直到到达棋盘的右下角。给定一格棋盘及上面的礼物，请计算最多能拿到多少价值的礼物。
```

```text
确定状态：
dp[i][j]表示到达(i,j)的各自时能拿到的礼物总和的最大值
转移方程：
