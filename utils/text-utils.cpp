...
bool ConfigLine::ParseLine(const std::string &line) {
  ...
  while (pos < size) {
	...
	std::cout << "No matching quote for " << my_quote << " in config line '"
                   << line << "'";
        return false;
      } else {
...
bool ConfigLine::GetValue(const std::string &key, float *value) {
  assert(value != NULL);
  std::map<std::string, std::pair<std::string, bool> >::iterator it = data_.begin();
  for (; it != data_.end(); ++it) {
    if (it->first == key) {
...
void ParseConfigLines(const std::vector<std::string> &lines,
  ...
  for (size_t i = 0; i < lines.size(); i++) {
    ...
    if (!ret) {
        std::cout << "Error parsing config line: " << lines[i];
    }
  }
}
